...
- Settings → Output device settings: replaces the "Force Max Volume" + "Apply ReplayGain" toggles with a single 3-state Volume picker (`None` / `ReplayGain` / `Force Max`). A single ⓘ hover tooltip sits to the left of the picker (after the "Volume" label) covering both modes. Force Max segment is visually disabled when Hog Mode is off; the binding-level guard remains as defense in depth. Same destructive-confirmation alert fires on transition into Force Max. Bit-perfect lingo moves from the Hog row label to the Force Max section of the tooltip ("Bit-perfect when EQ is off").
