...
- New `PlaybackCoordinator.nextReady` signal (+ `nextReadyUpdates` AsyncStream) — true when `queue.count >= 2 && queueNextEventId == queue[1].eventId`. Drives the popover's "loading next track" indicator independently from `PlaybackState`.
...
- The popover transport's loading indicator now appears in the **skip** slot (right) whenever the next track hasn't been preloaded into mpv, rather than replacing the play/pause icon. The play/pause button stays available throughout active playback so the user can always pause an actively-playing song. Skip is disabled while loading.
- `PlaybackState.loading` is no longer emitted from mid-playback paths (skipForward's queue[1]-not-queued branch, EOF-recovery defer, `tryQueueNextOrDefer` defer). Those flows now express "next preload pending" exclusively via the new `nextReady` signal. `.loading` is reserved for session startup (`play()` initial path) and the shallow-refetch interruption inside `skipForward()`.
- Extract `tryQueueNextOrDefer(_:)` helper on `PlaybackCoordinator` and convert 5 next-resolve call sites (`play`, `skipForward` shallow-refetch, `applyBitrateChange`, `handleSongPlaybackError`, `syncQueueHeadFromMpv`) to use it. Generalises PR 40's pattern: synchronous `cachedFile(for:)` probe replaces blocking `await songFileCache.localFile(...)`; on cache miss, queueNext is deferred and the existing `tryQueueNextIfPending(landed:)` hook fires when the download lands. Closes the deferred tech-debt item from PR 40.
...
- Play/pause button could flip to the loading spinner during normal playback (mid-track skip with uncached next, end-of-block transition with deferred queueNext, recovery after a song error), leaving the user unable to pause an actively-playing song. The "song plays but loading icon is still on" symptom is gone — slot 1 stays on the pause icon as long as mpv is producing audio.
- Playback no longer hangs after a song boundary when the next song's download is still in flight. Previously, the `.fileEnded(.eof)` recovery in the playback coordinator awaited the cache's `localFile(for:)` on the actor, which blocked on in-flight downloads. In rare cases (short promo + slow CDN response), this caused a cascade where audio went silent while the UI still showed "playing". Recovery now uses a synchronous cache check; if the file isn't ready, it defers the `queueNext` to the existing sequential downloader's post-download hook and emits `.loading` so the UI reflects reality.
