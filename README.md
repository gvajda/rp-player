...
- **Skip low-rated songs** (optional, off by default)
  ...
  - The Upcoming Program window still shows skip-bound songs, greyed-out and dimmed with a "⏭ SKIP" pill, so you can see what was filtered out.
...
- **Audio Unit plugins** (per device):
  - Run one Audio Unit effect (AUv2 `.component`) on playback — for example an Airwindows console or tape plugin. Settings → Audio Unit: turn Audio Unit on, import the `.component` (it's selected automatically), and open its controls with the sliders button.
...
- **Crossfeed** for headphones (per device):
  ...
  - Filter chain order is locked at **Preamp → EQ → Audio Unit → Crossfeed** so EQ acts on the source signal and crossfeed operates on the equalized (and plugin-processed) result.
...
- **Album art + ambient background**
  ...
  - The next two songs' art is **prefetched** as soon as the song queue is known (every `api/gapless` response carries the next ~20 songs), so the cover swap at song boundaries is instant — no blank tile, even on rapid skips.
...
- **Resilience**
  - Gapless song transitions. The next two songs are pre-downloaded to disk; mpv is then handed the queued local file via `loadfile … append-play`. At end-of-file mpv switches to the local file without rebuilding the audio device — no audible gap, even across format / sample-rate boundaries. The coordinator confirms each transition by reading mpv's `path` property (matched against both the remote `gaplessUrl` and the deterministic local cache path), so a redundant or out-of-order `MPV_EVENT_START_FILE` can't push the UI ahead of the audio.
  ...
  - Long-idle resume refetch: when resuming after a pause of 59 minutes or more, the app refetches the song queue via `api/gapless` instead of resuming whatever it was holding. The listener cursor at Radio Paradise advances on its own timeline, so a fresh fetch keeps the post-resume playback in sync with what the web/mobile players would show; it also covers the rare case where cached song files have been evicted by the LRU during the pause.
  - Resume stall watchdog: after the long-idle refetch hands mpv a freshly-downloaded local file, the app arms a 10-second watchdog on the first position update. If no audio frames arrive in time (decoder hang, AO-open hiccup, etc.) it stops and re-issues `engine.play` once. A second silent 10-second window surfaces *Playback stalled. Try Pause/Play to recover.* in the popover instead of leaving the listener staring at a frozen progress bar. Less likely to trigger now that mpv reads from local files instead of HTTP, but kept as defense-in-depth.

### Optimized performance

A menu-bar app should disappear into the background. RP Player aims for ~4% avg single CPU core footprint and ~80 MB RAM ceiling (measured on an M1 laptop during active FLAC playback).
...
- **HTTP demuxer buffers sized for radio, not video.** mpv's defaults (~150 MB forward buffer, ~75 MB backward) are tuned for video scrubbing. RP Player caps them at 32 MiB / 1 MiB with a 10-second cache window — enough to hold a whole FLAC song in the demuxer cache for snappier skip-forward, while staying well below the video-scrubbing defaults.
