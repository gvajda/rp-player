...
- Last merged (pending): **PR 39** — EQ Preset Editor. Per-preset edit panel inside Settings → Equalizer with grid bands — one row per band (Type / Frequency / Gain / Q + trash icon), plus an Add band button (cap 10). Filter-type dropdown covers Bypass / Peak / Low Shelf / High Shelf; Bypass = `enabled=false` and round-trips as `OFF` rows in the on-disk `.txt`. Save / Save As / Rename buttons; Rename updates all `AudioProfile.eqPresetName` references atomically and rolls back if the store throws. Live preview while editing via `EqEditingOverride` actor side-channel (`Sources/RPPlayer/Config/EqEditingOverride.swift`) — the audio filter binder reads the in-memory draft whenever it's set (regardless of which device's profile is active — shared-preset edits intentionally cascade by design) so keystrokes reach the lavfi chain in real time without writing to disk on every tick. `EqPresetStore.rename(from:to:)` API added; preset name cap dropped from 255 to 30 characters (UI label length is the binding constraint; longer import filenames truncate on first save). Parser preserves `OFF` rows as `enabled: false` (previously silently skipped); writer emits `OFF` rows with sequential `Filter N` indexing over all bands (enabled + disabled). `applyAudioFilterState` signature widened to consult the override actor; old `AudioFilterKey` struct removed (binder diffs directly against an override-aware key tuple). Read-only "show parsed values" eye icon dropped in favor of the always-editable panel. Save also shows a confirmation alert when the preset is referenced by 2+ audio profiles ("This preset is used by N audio devices…") so the cascade is opt-in; 1-device saves skip the alert. 540 tests.
