...
- Last merged (pending): **PR 40** — EOF recovery non-blocking. Fixes a silent-playback cascade where `.fileEnded(.eof)` recovery awaited `songFileCache.localFile(for: next)` on the actor — when the next song's download wasn't done yet (short promo + slow CDN), the actor block queued the promo's own EOF behind the wait and the queue/mpv state desynced (UI showed "playing", audio silent). Recovery now uses synchronous `cachedFile(for:)`; on cache miss it defers `queueNext` and emits `.loading`. New `tryQueueNextIfPending(landed:)` helper fires from `kickSequentialDownload` after each download lands — logs `elapsedSinceDeferMs`, lifts state from `.loading` → `.playing`. Cascade-protection lift at end of the recovery branch handles queue collapse to <2 before the queueNext branch could re-defer. Deferred: 5 other `await songFileCache.localFile(...)` sites in `PlaybackCoordinator` have the same blocking-actor risk — captured in `docs/pr-history.md` § Deferred. 543 tests.
