...
- Last merged (pending): **PR 42** — cache-aware transport buttons + stop action. Popover transport slot 2 (right of play/pause) renders three modes depending on `(isPlaying, nextReady, isPaused)`: skip button when next track is queued, loading spinner when playing-but-not-yet-queued, stop button (`stop.fill`) when paused. New `PlaybackCoordinator.nextReady` signal (`queue.count >= 2 && queueNextEventId == queue[1].eventId`) + `nextReadyUpdates` AsyncStream — updated at every `queue` / `queueNextEventId` mutation in `LivePlaybackCoordinator` via new `updateNextReady()` helper. Mid-playback `.loading` emits removed from `skipForward()` queue[1]-not-queued branch, `tryQueueNextOrDefer` defer, EOF-recovery defer, and `tryQueueNextIfPending` lift; `.loading` reserved for session startup (`play()` initial + `skipForward()` shallow refetch). Fixes "song plays but loading icon is still on" symptom — slot 1 stays on pause icon throughout active playback. `MiniPlayerViewModel.stopPlayback()` calls `coordinator.stop()`; view model's `.stopped` arm now clears nowPlaying / currentArt / lastLoadedCoverPath / ambientTopColor / currentRating / currentBitrateLabel / songElapsed / songDuration / lastNotifiedSongId (channel selection preserved). 5 position-related tests in `MiniPlayerViewModelTests` updated to fire `.playing` before `start()` (stateUpdates replay-on-subscribe now reaches the new `.stopped` clear). 6 coordinator tests rewritten to assert `nextReady` instead of removed `.loading` flicker. 559 tests.
