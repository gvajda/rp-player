...
- Last merged (pending): **PR 43** — EQ preset picker switch while editing. Clean picker change reseeds the editor; dirty change shows an "Unsaved EQ changes" alert (Keep editing / Discard / Save / Save as new preset…). New `SettingsViewModel.requestPresetSwitch(to:)` entry point + `pendingPresetSwitch` published state + 4 resolve methods. Save-as variant lands the picker on the user's originally-picked target, not on the save-as name. Internals: extracted private `writePresetFile(name:)` from `saveEditAs`; private `performSwitch(to:)` does the atomic picker change + editor reseed (includes a manual `self.eqPresetName = target` sync hop to avoid configStore-stream-lag reparsing the wrong preset). SwiftUI alert + name sheet attached to `eqSection`; `savingAsInProgress` `@State` flag gates the alert's `isPresented` setter so the Save-as button-tap doesn't auto-cancel the pending switch before the sheet's OK handler runs. 13 new tests on `SettingsViewModelEqEditTests`. 572 tests.
